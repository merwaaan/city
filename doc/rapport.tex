...
Imaginons un exemple trivial pour illustrer ce processus. On souhaite
calculer l'état que prendra la cellule $C$, au centre du quadrillage
de la figure \ref{fig:ca-example}. On commence par calculer les scores
de transition en fonction du voisinage de $C$.
...
classique (voir figure \ref{fig:ac}). On remarque deux
problèmes. Premièrement, l'état de l'automate change drastiquement en
...
chaque modification locale du système afin qu'elle puisse se
...
exprimer en fonction du temps une variation subissant un élan de
croissance en milieu de parcours. La sigmoïde classique (figure
...
obtenir une fonction associant une probabilité de changement d'état en
fonction de l'âge de la cellule considérée. Le facteur 0.02 permet
d'adoucir la pente de la fonction autour de 0 tandis que 350 sert à la
décaler $f(x)$ de façon à pouvoir l'utiliser dans le domaine positif.

AJOUTER LEGENDE SUR FIG SIGMOIDES
...
\begin{figure}[h]
  ...
  \caption{Six configurations du diagramme du diagramme de Voronoï
    cellulaire.}
  \label{fig:ac-stable}
\end{figure}

Plus la simulation avance et plus le système est chargé de parcelles à
haute densité. Un lectteur averti pourrait argumenter que ce
...
remplie de grands immeubles (pour simplifier). Mais il faut garder à
l'esprit que le mécanisme présenté dans cette section ne gère que la
croissance verticale. Par la suite, la croissance horizontale de la
ville modifiera et étendra sa structure de façon à ce que les
parcelles en bordures soient plus modestes.
...
sophistiqué pour améliorer le réalisme de la simulation. On pourrait,
par exemple, prendre en compte les valeurs financières des parcelles
ou les classes sociales des habitants.

\subsubsection{Placement des éléments potentiels}

Le second mécanisme place de nouvelles parcelles en bordure de la
ville et est ainsi responsable de sa croissance horizontale.

Les nouvelles parcelles placées bénéficient néanmoins d'un statut
spécial car elles sont considérées comme \textit{potentielles} et se
démarquent des parcelles \textit{construites} par leur impact sur
...
\item{Une parcelle potentielle peut accueillir des routes sur ses
  arêtes.}
\end{itemize}
...
n'existant pas encore et étant en réalité intangible.
...
l'enveloppe urbain et à s'éloigner des centres, non pas par animosité
...
\begin{figure}[h]
  \centering
  \subcaptionbox{}[.3\linewidth][c]{
    %\includegraphics[width=.3\linewidth]{images/field_density1.png}
  }
  \subcaptionbox{}[.3\linewidth][c]{
    %\includegraphics[width=.3\linewidth]{images/field_density2.png}
  }
  \caption{Le champ de vecteurs densitaire guide les nouvelles
    parcelles vers l'extérieur de la ville.}
  \label{fig:field-density}
\end{figure}

Il est naturel que les parcelles potentielles soient placées près des
installations routières existantes. À cet effet, on ajoute un champ
pour lequel chaque vecteur pointe vers la route construite la plus
proche.
...
\begin{figure}[h]
  ...
  \caption{Le champ de vecteurs viaire guide les nouvelles parcelles
    vers les routes construites.}
  \label{fig:field-road}
\end{figure}
...
restrictions quand à la direction que l'expansion d'une ville va
prendre. Parmi ces contraintes, on pense aux zones non constructibles
comme les plans d'eau, les fôrets mais aussi aux fortes pentes. Le
champ de vecteur étant dédié à l'évitement de telles zones guident
vers leur bordure mais, hors de leur aire, l'influence est nulle.
...
\begin{figure}[h]
  ...
  \caption{Le champ de vecteurs des obstacles évite que de nouvelles
    parcelles soient placées sur des zones interdites.}
  \label{fig:field-obstacle}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Le champ de vecteurs final est la somme pondérée de tous
    les autres champs d'influences.}
  \label{fig:field-sum}
\end{figure}
